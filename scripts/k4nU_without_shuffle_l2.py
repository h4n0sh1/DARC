...
if __name__=="__main__": 
    ...
    F = pd.DataFrame.from_dict(F_dict,orient="index")
    F = F.rename(columns=month_dict)
    F.to_csv("F_truth_without_shuffle.csv",index=True)
    ...
    dfn.to_csv("./k4nU_without_shuffle.csv",index=False)
    dfa=pd.read_csv("./k4nU_without_shuffle.csv")
    
    print("Checking Output format \n")
    ground_truth, submission = round1_preprocessing(
        "./ground_truth.csv", "./k4nU_without_shuffle.csv"
    )
    ...
    print(scores)
    with open('scores_k4nU_without_shuffle', 'wb') as fp:
