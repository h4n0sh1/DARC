...
if __name__=="__main__": 
    ...
    dfn.drop(["month","year","index"],axis=1,inplace=True)
    dfn=retransformDel(dfn)
    dfn.to_csv("./k4nU_without_hash.csv",index=False)
    dfa=pd.read_csv("./k4nU_without_hash.csv")
    
    print("Checking Output format \n")
    ground_truth, submission = round1_preprocessing(
        "./ground_truth.csv", "./k4nU_without_hash.csv"
    )
    ...
    print(scores)
    with open('scores_k4nU_without_hash', 'wb') as fp:
